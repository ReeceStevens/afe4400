...
// type SpiError = <core::fmt::Error as FullDuplex<u8>>::Error;

// impl<E> From<E> for nb::Error<E>
//     where E:  {
//     fn from(error: E) -> nb::Error<E> {
//         nb::Error::Other(error)
//     }
// }

// impl From<SpiError> for nb::Error<SpiError> {
...
impl<SPI: FullDuplex<u8>, IN: InputPin, OUT: OutputPin> Afe4400<SPI, IN, OUT> {
    pub fn writeData(&mut self, register: u8, data: u32) -> nb::Result<(), AfeError> {
    ...
    pub fn readData(&mut self, register: u8) -> nb::Result<u32, AfeError> {
        self.writeData(registers::CONTROL0, 0x01 as u32).unwrap();
        block!(self.spi.send(register));
        let mut register_data: u32 = 0;
        for _ in 0..3 {
            register_data = register_data << 8;
            register_data |= block!(self.spi.read()).ok().unwrap() as u32;
        }
        Ok(register_data)
    }

    fn getLedData(&mut self, led_register: u8) -> u32 {
    ...
    ///           256
    ///
    pub fn setLedCurrent(&mut self, value: u8) -> u32 {
        let both_leds = ((value as u32) << 8) + value;
        self.writeData(registers::LEDCNTRL, both_leds + 0x010000);
        self.readData(registers::LEDCNTRL)
    }

    pub fn defaultPulseTimings(&mut self) {
    ...
    pub fn self_check(&mut self) -> bool {
        ...
        self.readData(registers::CONTROL1) != 0
    }
